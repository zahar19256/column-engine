...
inline void TypedTopK(std::shared_ptr<ColumnT> column, std::vector<size_t>& index, std::vector<size_t>& classes,
                      size_t limit, SortDirection direction = SortDirection::Asc) {
    if (!column) {
    ...
    if (need == index.size()) {
        ...
        auto heap_compare = [&](size_t left_pos, size_t right_pos) { return better_with_tie(left_pos, right_pos); };
