...
enum class ExprType { Filter, Operator };

enum class PredicateOpExprType : int8_t { And = 0, Or, Xor, Not };
...
public:
    ...
    CaseWhenExpr(std::vector<std::pair<std::shared_ptr<PredicateExpr>, std::shared_ptr<ScalarExpr>>> cases,
                 std::shared_ptr<ScalarExpr> else_expr, ColumnType out_type = ColumnType::Unknown)
        : cases_(std::move(cases)), else_(std::move(else_expr)), out_type_(out_type) {}
...
private:
    ...
    static void PushTypedValue(const std::shared_ptr<Column>& result, const std::shared_ptr<Column>& source,
                               size_t row) {
    ...
    static void PushStringValue(const std::shared_ptr<Column>& result, const std::shared_ptr<Column>& source,
                                size_t row) {
    ...
    static void PushValue(const std::shared_ptr<Column>& result, const std::shared_ptr<Column>& source, size_t row,
                          ColumnType type) {
        switch (type) {
...
public:
    explicit UnaryExpr(std::shared_ptr<ScalarExpr> col, UnaryExprType type,
                       std::optional<std::string> arg1 = std::nullopt, std::optional<std::string> arg2 = std::nullopt)
...
public:
    explicit LiteralExpr(Utility::LiteralValue value, ColumnType type = ColumnType::Unknown)
        : value_(std::move(value)), type_(type == ColumnType::Unknown ? InferType(value_) : type) {}
...
public:
    BinaryExpr(std::shared_ptr<ScalarExpr> left, std::shared_ptr<ScalarExpr> right, BinaryExprType op,
               ColumnType out_type = ColumnType::Unknown)
        : op_(op), left_(std::move(left)), right_(std::move(right)), out_type_(out_type) {}
...
public:
    ...
    PredicateOpExpr(PredicateOpExprType type, std::unique_ptr<PredicateExpr> left,
                    std::unique_ptr<PredicateExpr> right = nullptr) {
        operator_type = type;
