...
enum class UnaryExprType : int8_t { Length = 0, ExtractMinute, RegexpReplace, DateFormatHour, DateTruncMinute };

template <typename T> struct IsDateOrTimeStampColumn : std::false_type {};

template <> struct IsDateOrTimeStampColumn<DateColumn> : std::true_type {};

template <> struct IsDateOrTimeStampColumn<TimeStampColumn> : std::true_type {};

template <typename T> inline constexpr bool IsDateOrTimeStampColumnV = IsDateOrTimeStampColumn<T>::value;
...
template <typename T>
concept NumericColumn =
    HasValueType<T> && (std::is_integral_v<typename T::ValueType> || std::is_floating_point_v<typename T::ValueType> ||
