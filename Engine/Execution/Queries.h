...
inline void
CollectColumns(const QueryNode& query,
               std::unordered_set<std::string>& column_names) { // TODO в будущем сделать static cast + GetType
...
inline std::unique_ptr<QueryNode> MakeFilterPlan(std::unique_ptr<QueryNode> child,
...
inline std::unique_ptr<QueryNode> MakeLimitPlan(std::unique_ptr<QueryNode> child, size_t limit, size_t offset = 0) {
...
inline std::unique_ptr<QueryNode> MakeOrderByLimitPlan(std::unique_ptr<QueryNode> child,
                                                       std::vector<std::shared_ptr<ScalarExpr>> order_by,
                                                       std::vector<SortDirection> directions, size_t limit) {
...
inline std::unique_ptr<QueryNode> MakeGroupByQueryPlan(const std::string& table_name,
...
inline std::unique_ptr<QueryNode> MakeGroupByOrderByLimitQueryPlan(const std::string& table_name,
                                                                   std::vector<std::shared_ptr<ScalarExpr>> group_by,
                                                                   std::vector<Aggregation::AggregationCall> aggregates,
                                                                   std::vector<std::shared_ptr<ScalarExpr>> order_by,
...
inline std::unique_ptr<QueryNode> MakeFilteredGroupByOrderByLimitQueryPlan(
    const std::string& table_name, std::unique_ptr<PredicateExpr> predicate,
    std::vector<std::shared_ptr<ScalarExpr>> group_by, std::vector<Aggregation::AggregationCall> aggregates,
    std::vector<std::shared_ptr<ScalarExpr>> order_by, std::vector<SortDirection> directions, size_t limit) {
    auto scan = std::make_unique<ScanNode>(table_name);
...
inline std::unique_ptr<QueryNode> MakeFilteredGroupByHavingOrderByLimitQueryPlan(
    const std::string& table_name, std::unique_ptr<PredicateExpr> predicate,
    std::vector<std::shared_ptr<ScalarExpr>> group_by, std::vector<Aggregation::AggregationCall> aggregates,
    std::unique_ptr<PredicateExpr> having, std::vector<std::shared_ptr<ScalarExpr>> order_by,
    std::vector<SortDirection> directions, size_t limit) {
    auto scan = std::make_unique<ScanNode>(table_name);
    auto filter = MakeFilterPlan(std::move(scan), std::move(predicate));
    auto aggregate = std::make_unique<AggregateNode>();
...
inline std::unique_ptr<QueryNode> MakeProjectionOrderByLimitQueryPlan(const std::string& table_name,
                                                                      std::vector<std::string> need_columns,
                                                                      std::vector<std::shared_ptr<ScalarExpr>> order_by,
                                                                      std::vector<SortDirection> directions,
                                                                      size_t limit) {
...
inline std::unique_ptr<QueryNode> MakeFilteredProjectionOrderByLimitQueryPlan(
    const std::string& table_name, std::unique_ptr<PredicateExpr> predicate, std::vector<std::string> need_columns,
    std::vector<std::shared_ptr<ScalarExpr>> order_by, std::vector<SortDirection> directions, size_t limit) {
    auto scan = std::make_unique<ScanNode>(table_name);
    auto filter = MakeFilterPlan(std::move(scan), std::move(predicate));
...
inline std::shared_ptr<Executor> BuildExecutor(const QueryNode& query) {
    ...
    if (const auto* order_by = dynamic_cast<const OrderByNode*>(&query)) {
        ...
        auto executor = std::make_shared<OrderByExecutor>(order_by->order_by_, order_by->limit_, order_by->directions_);
...
inline std::unique_ptr<QueryNode> MakeSecondQueryPlan(const std::string& table_name) {
    ...
    auto filter = std::make_unique<FilterNode>();
    filter->predicates_ = std::shared_ptr<PredicateExpr>(MakeFilter({"AdvEngineID", "0", Filters::OpType::NotEqual}));
...
inline std::unique_ptr<QueryNode> MakeThirdQueryPlan(const std::string& table_name) {
    ...
    auto aggregate = std::make_unique<AggregateNode>();
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Sum, std::string("AdvEngineID"),
                                                  kQuery3SumAdvEngineIdColumn, ColumnType::Int16, ColumnType::Int64));
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, kQuery3CountColumn,
                                                  ColumnType::Unknown, ColumnType::Int64));
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Avg, std::string("ResolutionWidth"),
                                                  kQuery3AvgResolutionWidthColumn, ColumnType::Int16,
                                                  ColumnType::Int16));
    aggregate->next_nodes_.push_back(std::move(scan));
...
inline std::unique_ptr<QueryNode> MakeFourthQueryPlan(const std::string& table_name) {
    ...
    auto aggregate = std::make_unique<AggregateNode>();
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Avg, std::string("UserID"),
                                                  kQuery4ResultColumn, ColumnType::Int64, ColumnType::Int64));
    aggregate->next_nodes_.push_back(std::move(scan));
...
inline std::unique_ptr<QueryNode> MakeFifthQueryPlan(const std::string& table_name) {
    ...
    auto aggregate = std::make_unique<AggregateNode>();
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Distinct, std::string("UserID"),
                                                  kQuery5ResultColumn, ColumnType::Int64, ColumnType::Int64));
    aggregate->next_nodes_.push_back(std::move(scan));
...
inline std::unique_ptr<QueryNode> MakeSixthQueryPlan(const std::string& table_name) {
    ...
    auto aggregate = std::make_unique<AggregateNode>();
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Distinct, std::string("SearchPhrase"),
                                                  kQuery6ResultColumn, ColumnType::String, ColumnType::Int64));
    aggregate->next_nodes_.push_back(std::move(scan));
...
inline std::unique_ptr<QueryNode> MakeSeventhQueryPlan(const std::string& table_name) {
    ...
    auto aggregate = std::make_unique<AggregateNode>();
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Min, std::string("EventDate"),
                                                  kQuery7MinEventDateColumn, ColumnType::Date, ColumnType::Date));
    aggregate->aggregates_.push_back(MakeAggrCall(Aggregation::AggregationType::Max, std::string("EventDate"),
                                                  kQuery7MaxEventDateColumn, ColumnType::Date, ColumnType::Date));
    aggregate->next_nodes_.push_back(std::move(scan));
...
inline std::unique_ptr<PredicateExpr> MakeAndPredicate(std::unique_ptr<PredicateExpr> left,
...
inline std::unique_ptr<PredicateExpr> MakeOrPredicate(std::unique_ptr<PredicateExpr> left,
                                                      std::unique_ptr<PredicateExpr> right) {
    return std::make_unique<PredicateOpExpr>(PredicateOpExprType::Or, std::move(left), std::move(right));
}

inline std::unique_ptr<PredicateExpr> MakeInPredicate(const std::string& column_name,
...
inline std::unique_ptr<PredicateExpr> MakeJulyCounterPredicate(bool dont_count_hits, bool require_url,
                                                               bool require_title = false) {
    auto result = MakeFilter({"CounterID", "62", Filters::OpType::Equal});
    result =
        MakeAndPredicate(std::move(result), MakeFilter({"EventDate", "2013-07-01", Filters::OpType::GreaterOrEqual}));
...
inline std::vector<Aggregation::AggregationCall> CountAs(std::string alias = "c") {
    ...
    result.push_back(MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, std::move(alias),
                                  ColumnType::Unknown, ColumnType::Int64));
    return result;
}
...
inline std::unique_ptr<QueryNode> MakeEighthQueryPlan(const std::string& table_name) {
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeFilter({"AdvEngineID", "0", Filters::OpType::NotEqual}),
        {MakeColumnExpr("AdvEngineID", ColumnType::Int16)}, CountAs("c"), {MakeColumnExpr("c", ColumnType::Int64)},
        {SortDirection::Desc}, std::numeric_limits<size_t>::max());
}

inline std::unique_ptr<QueryNode> MakeNinthQueryPlan(const std::string& table_name) {
    std::vector<Aggregation::AggregationCall> aggregates;
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Distinct, std::string("UserID"), "u",
                                      ColumnType::Int64, ColumnType::Int64));
    return MakeGroupByOrderByLimitQueryPlan(table_name, {MakeColumnExpr("RegionID", ColumnType::Int64)},
                                            std::move(aggregates), {MakeColumnExpr("u", ColumnType::Int64)},
                                            {SortDirection::Desc}, 10);
}

inline std::unique_ptr<QueryNode> MakeTenthQueryPlan(const std::string& table_name) {
    std::vector<Aggregation::AggregationCall> aggregates;
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Sum, std::string("AdvEngineID"),
                                      "sum_adv_engine_id", ColumnType::Int16, ColumnType::Int64));
    aggregates.push_back(
        MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, "c", ColumnType::Unknown, ColumnType::Int64));
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Avg, std::string("ResolutionWidth"),
                                      "avg_resolution_width", ColumnType::Int16, ColumnType::Int16));
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Distinct, std::string("UserID"), "uniq_user_id",
                                      ColumnType::Int64, ColumnType::Int64));
    return MakeGroupByOrderByLimitQueryPlan(table_name, {MakeColumnExpr("RegionID", ColumnType::Int64)},
                                            std::move(aggregates), {MakeColumnExpr("c", ColumnType::Int64)},
                                            {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeEleventhQueryPlan(const std::string& table_name) {
    ...
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Distinct, std::string("UserID"), "u",
                                      ColumnType::Int64, ColumnType::Int64));
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("MobilePhoneModel"), {MakeColumnExpr("MobilePhoneModel", ColumnType::String)},
        std::move(aggregates), {MakeColumnExpr("u", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeTwelfthQueryPlan(const std::string& table_name) {
    ...
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Distinct, std::string("UserID"), "u",
                                      ColumnType::Int64, ColumnType::Int64));
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("MobilePhoneModel"),
...
inline std::unique_ptr<QueryNode> MakeThirteenthQueryPlan(const std::string& table_name) {
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("SearchPhrase"), {MakeColumnExpr("SearchPhrase", ColumnType::String)},
        CountAs("c"), {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeFifteenthQueryPlan(const std::string& table_name) {
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("SearchPhrase"),
...
inline std::unique_ptr<QueryNode> MakeSixteenthQueryPlan(const std::string& table_name) {
    return MakeGroupByOrderByLimitQueryPlan(table_name, {MakeColumnExpr("UserID", ColumnType::Int64)}, CountAs("c"),
                                            {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeSeventeenthQueryPlan(const std::string& table_name) {
    return MakeGroupByOrderByLimitQueryPlan(
        table_name, {MakeColumnExpr("UserID", ColumnType::Int64), MakeColumnExpr("SearchPhrase", ColumnType::String)},
        CountAs("c"), {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeTwentySecondQueryPlan(const std::string& table_name) {
    ...
    std::vector<Aggregation::AggregationCall> aggregates;
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Min, std::string("URL"), "min_url",
                                      ColumnType::String, ColumnType::String));
    aggregates.push_back(
        MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, "c", ColumnType::Unknown, ColumnType::Int64));
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, std::move(predicate), {MakeColumnExpr("SearchPhrase", ColumnType::String)}, std::move(aggregates),
        {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeTwentyThirdQueryPlan(const std::string& table_name) {
    ...
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Min, std::string("URL"), "min_url",
                                      ColumnType::String, ColumnType::String));
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Min, std::string("Title"), "min_title",
                                      ColumnType::String, ColumnType::String));
    aggregates.push_back(
        MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, "c", ColumnType::Unknown, ColumnType::Int64));
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Distinct, std::string("UserID"), "u",
...
inline std::unique_ptr<QueryNode> MakeTwentyFourthQueryPlan(const std::string& table_name) {
    return MakeFilteredProjectionOrderByLimitQueryPlan(
        table_name, MakeFilter({"URL", "%google%", Filters::OpType::Like}), AllHitsColumns(),
        {MakeColumnExpr("EventTime", ColumnType::Timestamp)}, {SortDirection::Asc}, 10);
}

inline std::unique_ptr<QueryNode> MakeTwentyFifthQueryPlan(const std::string& table_name) {
    return MakeFilteredProjectionOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("SearchPhrase"), {"SearchPhrase"},
        {MakeColumnExpr("EventTime", ColumnType::Timestamp)}, {SortDirection::Asc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeTwentySixthQueryPlan(const std::string& table_name) {
    return MakeFilteredProjectionOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("SearchPhrase"), {"SearchPhrase"},
        {MakeColumnExpr("SearchPhrase", ColumnType::String)}, {SortDirection::Asc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeTwentySeventhQueryPlan(const std::string& table_name) {
    return MakeFilteredProjectionOrderByLimitQueryPlan(
        ...
        {SortDirection::Asc, SortDirection::Asc}, 10);
}

inline std::unique_ptr<QueryNode> MakeTwentyEighthQueryPlan(const std::string& table_name) {
    std::vector<Aggregation::AggregationCall> aggregates;
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Avg,
                                      MakeLengthExpr(MakeColumnExpr("URL", ColumnType::String)), "l", ColumnType::Int64,
                                      ColumnType::Int64));
    aggregates.push_back(
        MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, "c", ColumnType::Unknown, ColumnType::Int64));
    return MakeFilteredGroupByHavingOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("URL"), {MakeColumnExpr("CounterID", ColumnType::Int64)},
        std::move(aggregates), MakeFilter({"c", "100000", Filters::OpType::Greater}),
        {MakeColumnExpr("l", ColumnType::Int64)}, {SortDirection::Desc}, 25);
}

inline std::shared_ptr<ScalarExpr> MakeTwentyNinthQueryKeyExpr() {
    return MakeRegexpReplaceExpr(MakeColumnExpr("Referer", ColumnType::String), R"(^https?://(?:www\.)?([^/]+)/.*$)",
                                 R"(\1)");
}
...
inline std::unique_ptr<QueryNode> MakeTwentyNinthQueryPlan(const std::string& table_name) {
    std::vector<Aggregation::AggregationCall> aggregates;
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Avg,
                                      MakeLengthExpr(MakeColumnExpr("Referer", ColumnType::String)), "l",
                                      ColumnType::Int64, ColumnType::Int64));
    aggregates.push_back(
        MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, "c", ColumnType::Unknown, ColumnType::Int64));
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Min, std::string("Referer"), "min_referer",
                                      ColumnType::String, ColumnType::String));
    auto order = MakeFilteredGroupByHavingOrderByLimitQueryPlan(
        table_name, MakeNotEmptyPredicate("Referer"), {MakeTwentyNinthQueryKeyExpr()}, std::move(aggregates),
        MakeFilter({"c", "100000", Filters::OpType::Greater}), {MakeColumnExpr("l", ColumnType::Int64)},
        {SortDirection::Desc}, 25);
    auto projection =
        MakeProjectionPlan(std::move(order), {"group_0", "l", "c", "min_referer"}, {"k", "l", "c", "min_referer"});
    PushRequiredColumnsToScans(*projection);
    return projection;
}
...
inline std::unique_ptr<QueryNode> MakeThirtyFirstQueryPlan(const std::string& table_name) {
    ...
    aggregates.push_back(
        MakeAggrCall(Aggregation::AggregationType::Count, std::nullopt, "c", ColumnType::Unknown, ColumnType::Int64));
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Sum, std::string("IsRefresh"), "sum_is_refresh",
    ...
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        ...
        std::move(aggregates), {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeThirtyThirdQueryPlan(const std::string& table_name) {
    ...
    aggregates.push_back(MakeAggrCall(Aggregation::AggregationType::Avg, std::string("ResolutionWidth"),
                                      "avg_resolution_width", ColumnType::Int16, ColumnType::Int16));
    return MakeGroupByOrderByLimitQueryPlan(
        table_name, {MakeColumnExpr("WatchID", ColumnType::Int64), MakeColumnExpr("ClientIP", ColumnType::Int64)},
        std::move(aggregates), {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}

inline std::unique_ptr<QueryNode> MakeThirtyFourthQueryPlan(const std::string& table_name) {
    return MakeGroupByOrderByLimitQueryPlan(table_name, {MakeColumnExpr("URL", ColumnType::String)}, CountAs("c"),
                                            {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}

inline std::unique_ptr<QueryNode> MakeThirtyFifthQueryPlan(const std::string& table_name) {
    return MakeGroupByOrderByLimitQueryPlan(
        table_name, {MakeLiteralExpr(int64_t{1}, ColumnType::Int64), MakeColumnExpr("URL", ColumnType::String)},
        CountAs("c"), {MakeColumnExpr("c", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}

inline std::shared_ptr<ScalarExpr> MakeClientIPMinus(int64_t value) {
    return MakeSubExpr(MakeColumnExpr("ClientIP", ColumnType::Int64), MakeLiteralExpr(value, ColumnType::Int64),
                       ColumnType::Int64);
}

inline std::unique_ptr<QueryNode> MakeThirtySixthQueryPlan(const std::string& table_name) {
    return MakeGroupByOrderByLimitQueryPlan(table_name,
                                            {
                                            ...
                                            CountAs("c"), {MakeColumnExpr("c", ColumnType::Int64)},
                                            {SortDirection::Desc}, 10);
}

inline std::unique_ptr<QueryNode> MakeThirtySeventhQueryPlan(const std::string& table_name) {
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeJulyCounterPredicate(true, true), {MakeColumnExpr("URL", ColumnType::String)},
        CountAs("PageViews"), {MakeColumnExpr("PageViews", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}

inline std::unique_ptr<QueryNode> MakeThirtyEighthQueryPlan(const std::string& table_name) {
    return MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeJulyCounterPredicate(true, false, true), {MakeColumnExpr("Title", ColumnType::String)},
        CountAs("PageViews"), {MakeColumnExpr("PageViews", ColumnType::Int64)}, {SortDirection::Desc}, 10);
}
...
inline std::unique_ptr<QueryNode> MakeThirtyNinthQueryPlan(const std::string& table_name) {
    ...
    auto order = MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, std::move(predicate), {MakeColumnExpr("URL", ColumnType::String)}, CountAs("PageViews"),
        {MakeColumnExpr("PageViews", ColumnType::Int64)}, {SortDirection::Desc}, 1010);
...
inline std::shared_ptr<ScalarExpr> MakeFortiethQuerySrcExpr() {
    ...
    cases.emplace_back(std::shared_ptr<PredicateExpr>(std::move(predicate)),
                       MakeColumnExpr("Referer", ColumnType::String));
    return MakeCaseWhenExpr(std::move(cases), MakeLiteralExpr(std::string(""), ColumnType::String), ColumnType::String);
}

inline std::unique_ptr<QueryNode> MakeFortiethQueryPlan(const std::string& table_name) {
    auto order = MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeJulyCounterPredicate(false, false),
        {
        ...
        CountAs("PageViews"), {MakeColumnExpr("PageViews", ColumnType::Int64)}, {SortDirection::Desc}, 1010);
    return MakeLimitPlan(std::move(order), 10, 1000);
}
...
inline std::unique_ptr<QueryNode> MakeFortyFirstQueryPlan(const std::string& table_name) {
    ...
    predicate = MakeAndPredicate(std::move(predicate),
                                 MakeFilter({"RefererHash", "3594120000172545465", Filters::OpType::Equal}));
    auto order = MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, std::move(predicate),
        {MakeColumnExpr("URLHash", ColumnType::Int64), MakeColumnExpr("EventDate", ColumnType::Date)},
        CountAs("PageViews"), {MakeColumnExpr("PageViews", ColumnType::Int64)}, {SortDirection::Desc}, 110);
...
inline std::unique_ptr<QueryNode> MakeFortySecondQueryPlan(const std::string& table_name) {
    ...
    predicate =
        MakeAndPredicate(std::move(predicate), MakeFilter({"URLHash", "2868770270353813622", Filters::OpType::Equal}));
    auto order = MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, std::move(predicate),
        {MakeColumnExpr("WindowClientWidth", ColumnType::Int16),
         MakeColumnExpr("WindowClientHeight", ColumnType::Int16)},
        CountAs("PageViews"), {MakeColumnExpr("PageViews", ColumnType::Int64)}, {SortDirection::Desc}, 10010);
...
inline std::unique_ptr<PredicateExpr> MakeFortyThirdQueryPredicate() {
    auto result = MakeFilter({"CounterID", "62", Filters::OpType::Equal});
    result =
        MakeAndPredicate(std::move(result), MakeFilter({"EventDate", "2013-07-14", Filters::OpType::GreaterOrEqual}));
...
inline std::unique_ptr<QueryNode> MakeFortyThirdQueryPlan(const std::string& table_name) {
    ...
    auto order = MakeFilteredGroupByOrderByLimitQueryPlan(
        table_name, MakeFortyThirdQueryPredicate(), {minute}, CountAs("PageViews"),
        {MakeColumnExpr("group_0", ColumnType::Timestamp)}, {SortDirection::Asc}, 1010);
...
inline Query3Result RunThirdQuery(const std::string& table_name) {
    ...
    if (!sum_adv_engine_id || !count || !avg_resolution_width || sum_adv_engine_id->Size() != 1 || count->Size() != 1 ||
        avg_resolution_width->Size() != 1) {
...
inline Query7Result RunSeventhQueryEventDateMinMax(const std::string& table_name) {
    ...
    if (!min_event_date || !max_event_date || min_event_date->Size() != 1 || max_event_date->Size() != 1) {
