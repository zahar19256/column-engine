...
public:
    ScanExecutor(const std::string& table_name, const std::vector<std::string>& column_names)
        : reader_(table_name), column_names_(column_names) {}
...
public:
    explicit FilterExecutor(std::shared_ptr<PredicateExpr> filter_expr) : filter_expr_(std::move(filter_expr)) {}
...
protected:
    explicit AggregationExecutorBase(std::vector<Aggregation::AggregationCall> calls) : calls_(std::move(calls)) {}
        ...
        while (child->Next(input)) {
            ...
            for (size_t i = 0; i < states_.size(); ++i) {
                const std::shared_ptr<Column> column =
                    calls_[i].expression ? calls_[i].expression->EvalBatch(input, env) : nullptr;
...
public:
    explicit ProjectionExecutor(std::vector<std::string> need_columns,
...
public:
    explicit OrderByExecutor(std::vector<std::shared_ptr<ScalarExpr>> order_expr, size_t limit,
                             std::vector<SortDirection> directions = {})
        : order_expr_(std::move(order_expr)), directions_(std::move(directions)), limit_(limit) {}
    bool Next(Batch& data) override;
...
private:
    std::shared_ptr<Column> SliceColumn(std::shared_ptr<Column> column, size_t del, Utility::StringArena* arena,
                                        bool front = true);
    size_t limit_;
    size_t offset_ = 0;
    size_t skipped_ = 0;
