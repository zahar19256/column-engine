...
template <> struct ColumnTraits<__int128_t> {
...
template <typename T> inline ColumnType GetColumnType() {
...
template <typename T> inline T ParseInteger(const char* start, size_t size) {
    T result = 0;
...
template <typename T> inline T ParseInteger(const std::string& value) {
...
template <typename T> inline T ConvertDataFromString(const std::string& val) {
    if constexpr (std::is_same_v<T, std::string>) {
        return val;
    }
    if constexpr (std::is_same_v<T, double>) {
