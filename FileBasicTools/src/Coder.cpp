...
void Dictionary::Apply(std::shared_ptr<FlatStringColumn> col, EncodedColumn& result) {
    ...
    unique_string.reserve(col_size);
    offset.reserve(col_size);
    ...
    for (size_t i = 0; i < col_size; ++i) {
        ...
        if (inserted) {
            ...
            int32_t current_offset = (last == 0) ? static_cast<int32_t>(current_str.size())
                                                 : offset.back() + static_cast<int32_t>(current_str.size());
    ...
    size_t total_size =
        sizeof(int16_t) + (sizeof(int32_t) * last) + total_strings_length + (sizeof(int16_t) * col_size);
