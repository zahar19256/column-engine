...
protected:
    ...
    void CompareCSVFiles(const std::string& path1, const std::string& path2) {
        ...
        ASSERT_EQ(table1.size(), table2.size()) << "Row count mismatch between original and converted CSV";

        for (size_t i = 0; i < table1.size(); ++i) {
            ASSERT_EQ(table1[i].Size(), table2[i].Size()) << "Column count mismatch at row " << i;

            for (size_t j = 0; j < table1[i].Size(); ++j) {
                EXPECT_EQ(table1[i][j], table2[i][j]) << "Value mismatch at row " << i << ", col " << j;
            }
        }
    }
};
...
TEST_F(BelZConvertorTest, RoundTrip_Small) {
    ...
    ASSERT_TRUE(fs::exists(csvOutput)) << "Final CSV file not created. Expected: " << csvOutput;
